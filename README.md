...
- 포인트 증/차감에 실패하는 경우, 에러를 나타내야 할 때가 있고 아닐 때가 있다.
  - 예를 들면, 글 쓰기에서, 글 쓰는 사람에게 포인트 차감이 설정을 하면, 포인트가 모자라면 글을 쓸 수 없다.
  - 하지만, 글 쓰는 사람에게 포인트 증가를 하는 경우, 시간 제한에 걸리면, 포인트는 증가하지 않지만 글을 쓸 수 있다.
  자세한 것은 아래의 포인트 증/감 규정을 참고한다.
...
- 로그인 포인트는 로그인을 할 때에만 포인트 증가가 발생한다.
  ...
  대신, 출석 게시판에 제한을 24/1(24시간에 1번) 또는 1일 1회로 제한 해서, 특정 포인트를 주게한다.
  
- 포인트 증/감 제한
  추천 / 비추천은 전체 설정만 있고, 게시판 별 설정은 없다. 즉, 전체 설정이 모든 게시판의 추천/비추천에 적용되며, 회원 프로필에도 적용된다.
  게시판 별 설정은 전체 설정이 없다. 그냥 게시판 별로 설정을 해야 한다.
  
  모든 포인트 증/감에는 "시간/수" 제한과 "일/수" 제한이 있다.
  
  시간/수 제한: 5/6 과 같이 하면 5시간에 6번 할 수 있다.
  일/수 제한: '추가로 1일 6회로 제한하면'
...
- 추천을 받는 경우 반드시 0 또는 양의 정수 값만 입력한다.
  - 추천을 하는 경우, (0, 양수, 음수 상관없이) 정수를 입력한다.
- 반대를 받는 경우, 반드시 0 또는 음의 정수만 입력한다.
  - 추천을 하는 경우, (0, 양수, 음수 상관없이) 정수를 입력한다.
  
- 게시판 새글을 작성하는 경우, (0, 양수, 음수) 상관없이 정수를 입력한다.
  즉, 유료 게시판의 경우, 게시판에 글 작성 포인트를 음수로 두어, 사용자가 글 작성시 포인트를 차감 시킬 수 있다.
  
- 포인트나 글 관련하여 포인트 증/감을 할 때, 제한을 둘 수 있다.
  - 제한은 '시간 별 몇 회, 하루에 몇 회'로 2가지를 조합하여 사용 할 수 있다.
  - 추천/비추천은 제한에 걸리면 포인트 증/감을 하지 않지만, 추천/비추천은 계속 할 수 있다.
  - 글/코멘트 관련 포인트 증/감도 마찬가지로 제한에 걸리면 포인트가 증/감하지 않지만, 글/코멘트를 작성하거나 삭제 할 수 있다.
  즉, 포인트 제한에 걸리면 포인트 증/감은 하지 않지만, 관련 작업은 계속 진행된다.

- 추천/비추천을 받는 경우, 포인트가 감소한다면,
  추천/비추천 받는 사람이 포인트가 모자라도 추천/비추천을 받을 수 있으며, 이 때, 포인트가 음수로 내려가지 않고 최소 0이 된다.
  
- 추천/비추천을 하는 경우, 포인트가 모자라면, 할 수 없다.
  예를 들어 추천/비추천을 하는데, 포인트가 차감된다면, 차감되는 포인트 만큼 포인트를 가지고 있어야 한다. 그렇지 않으면 추천/비추천을 할 수 없다.
  추천/비추천 받는 경우는 상관없다. 특히 비추천을 받는 경우, 포인트가 0이어도 계속해서 비추천을 받을 수 있다.
...
- 추천/비추천을 한 경우, 처음 한번만 포인트 증/감을 하고, 그 다음에 취소 또는 반대로 추천/비추천을 해도 포인트가 복구되거나 포인트가 변하지 않는다.
  예를 들어, A 가 B 에게 추천을 했는데, B 가 포인트를 모두 써 버려, A 가 취소하려는데, B 포인트가 없는데, A 의 포인트를 복구한다면, 시스템적으로 포인트 손실이 생기기 때문이다.
  A 가 B 에게 추천을 했다가 취소 한 후, 다시 추천해도 포인트가 증/감하지 않는다.
  추천 취소를 할 때, 추천 기록 레코드를 DB에서 삭제하지 않고, C 로 두기 때문에 가능하다.
  
- 추천/비추천은 내가 포인트가 없어도 할 수 있다. 그냥 포인트 증/감 적용이 안 될 뿐이다.
  다시 말하면, 추천할 때, -50 포인트가 차감되는데, 내 포인트가 -50이 안된다면, 나와 상대방의 포인트에 아무런 변화가 없다. 다만, 추천은 된다.
  비 추천도 마찬가지이다. 비추천을 할 때, 내가 포인트가 모자라면, 나와 상대방의 포인트에 아무런 변화가 없지만, 비추천 자체는 된다.
  
  하지만, 글/코멘트를 쓰거나 삭제를 하려고 하는 경우, 포인트가 차감이 된다면, 그리고 내 포인트가 모자란다면, 글/코멘트 쓰기/삭제가 안된다.
...
- 시간/수, 일/수 제한에 걸리면 포인트는 증/감하지 않지만, 글/코멘트 쓰기/삭제는 계속 할 수 있다.
  이 때, '글/코멘트에 제한' 옵션을 '예'로 선택하면, 시간/수, 일/수 제한에 걸리면, 글/코멘트 쓰기/삭제를 못하게 한다. 즉, 글 쓰기를 제한하고자 할 때 "예"를 선택하면 된다.
  '아니오'를 선택하면, 포인트가 증/감하지 않지만, 글/코멘트 쓰기/삭제를 계속 할 수 있다.
...
- 장터에 하루에 1번만 글 쓰기를 할 때, 상단에 자신의 글을 표시하기 위해서, 삭제하고 다시 글을 쓸 수 있다. 이를 방지하기 위해서,
  - 글/코멘트 쓰기를 할 때에는 시간/수, 일/수 제한이 적용된다. 
    하지만, 글/코멘트 삭제를 할 때에는 적용이 되지 않는다.
    즉, 삭제하는 것은 마음데로 얼마든지 할 수 있지만, 글/코멘트 쓰기는 하루에 한번만 할 수 있다. 조금 전에 쓴 글을 삭제하고 다시 써도 안된다. 조금전에 글을 썼기 때문에, 삭제를 해도, 또 쓰지 못한다.
