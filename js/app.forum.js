const forumMixin = {
  ...
  methods: {
    ...
    onCommentEditFormSubmit(event) {
      this.$data.commentEditForm.comment_post_ID =
        event.target.elements.comment_post_ID.value;

      /// will have value if creating comment
      if (event.target.elements.comment_parent) {
        this.$data.commentEditForm.comment_parent =
          event.target.elements.comment_parent.value;
      }
      /// will have value if updating comment
      if (event.target.elements.comment_ID) {
        this.$data.commentEditForm.comment_ID =
          event.target.elements.comment_ID.value;
        this.$data.commentEditForm.comment_content =
          event.target.elements.comment_content.value;
      }
